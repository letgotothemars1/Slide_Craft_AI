...
Сервис для автоматической генерации презентаций на основе текстового промпта. Пользователь описывает тему — система создаёт структурированную презентацию и отдаёт готовый PDF.
...
**Бэкенд:** Python, FastAPI, SQLAlchemy, PostgreSQL (Supabase)
...
**Хранилище:** Supabase Storage (с fallback на локальную папку)
...
- **Формат слайдов 16:9** — слайды переведены с вертикального A4 на горизонтальный 16:9, как в PowerPoint/Google Slides; тёмная тема рендерит фон на весь слайд.
- **Безопасность секретов** — `.env.*` (включая бэкапы) добавлены в `.gitignore`, чтобы реальные ключи не попадали в репозиторий; `.env.example` остаётся в репо как шаблон.
- **Мониторинг production** — три уровня наблюдения: внешний uptime (UptimeRobot → `/health`), watchdog ресурсов сервера (Disk/RAM/CPU → Telegram) и проверка ежедневного бэкапа БД (dead man's switch → Telegram). Скрипты и инструкция в [`ops/`](ops/MONITORING.md).

## Мониторинг

Три независимых уровня наблюдения за production (`slidecraft.org`). Алерты по ресурсам и бэкапам шлются в отдельного admin-бота в Telegram.

| Уровень | Что ловит | Чем |
|---------|-----------|-----|
| Внешний uptime | сайт недоступен целиком, истёк SSL | UptimeRobot — HEAD `/health` каждые 5 мин |
| Ресурсы сервера | переполнение диска, утечка памяти, перегрузка CPU | `ops/slidecraft-watchdog.sh` (cron `*/5`) |
| Бэкапы БД | ночной `pg_dump` перестал создаваться / битый | `ops/slidecraft-backup-check.sh` (cron `0 9`) |

Подробности установки, пороги и ручная проверка — в [`ops/MONITORING.md`](ops/MONITORING.md).
